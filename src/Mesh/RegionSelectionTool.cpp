...
namespace GemCraft {
    ...
    std::set<size_t> FindNRingFaces(Neighbor_query& neighborQuery, const Region_growing::Region_map& map, CGAL::SM_Face_index queryFace, uint32_t nRing);
    ...
    void RegionSelectionTool::SelectRegion()
    {
        ...
        for (size_t iter = 0; iter < 6; iter++) {
            ...
            for (auto& i : selectedRegions) {
                for (auto& face : regions[i].second) {
                    auto item = CGAL::SM_Face_index(face);
                    std::set<size_t> result = FindNRingFaces(neighborQuery, map, item, 5);
                    for (auto& j : result) {

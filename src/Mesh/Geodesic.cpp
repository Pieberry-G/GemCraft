...
namespace GemCraft {
	...
	Face_location Geodesic::LocatePoint(CGALPoint point)
	{
	...
	glm::vec3 Geodesic::CalculateNormal(const glm::vec3& position)
	{
		...
		Face_location face_loc = LocatePoint(point);
	...
	Path Geodesic::ConstructGeodesicPath()
	{
		...
		Face_location start_path_loc = LocatePoint(start_path);
		// Get the target point
		glm::vec3 endPath = polyscope::state::endPath;
		CGALPoint end_path(endPath.x, endPath.y, endPath.z);
		Face_location end_path_loc = LocatePoint(end_path);
	...
	std::vector<Path> Geodesic::CalculateParallelPaths(const Path& path, int numberOfPaths, float pathSpacing)
	{
		...
		for (size_t count = 0; count < numberOfPaths; count++) {
			...
			for (size_t i = 0; i < len; i++) {
				...
				Face_location start_point_loc = LocatePoint(start_point);

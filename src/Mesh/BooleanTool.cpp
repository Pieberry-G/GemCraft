...
namespace GemCraft {
	...
	std::shared_ptr<CGALMesh> BooleanTool::ConstructContexHull(std::shared_ptr<CGALMesh>& mesh)
	{
		...
		//CGALpmp::border_halfedges(faces(*contexHull), *contexHull, boost::make_function_output_iterator([&](const halfedge_descriptor& h) {
		//	border.push_back(edge(h, *contexHull));
		//}));
